...
public class Portal121Compat extends org.bukkit.plugin.java.JavaPlugin {
    ...
    private <T> void reloadListener(Class<T> listenerClass, String configKey) {
        ...
        boolean alreadyRegistered = getServer().getPluginManager()
                .getPluginListeners(this).stream()
        ...
        if (enabled && !alreadyRegistered) {
            try {
                Object listener = listenerClass
                        ...
                        .newInstance(this);
                getServer().getPluginManager().registerEvents(
                        (org.bukkit.event.Listener) listener, this);
            ...
            // 注销已注册的监听器（通过移除所有该类型的已注册监听器）
            getServer().getPluginManager().getPluginListeners(this).removeIf(l -> {
