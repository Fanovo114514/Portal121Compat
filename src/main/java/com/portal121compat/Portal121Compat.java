...
public class Portal121Compat extends org.bukkit.plugin.java.JavaPlugin {
    ...
    public void onEnable() {
        ...
        // 注册实验性功能监听器（同时加入 activeListeners 以支持热重载）
        if (getConfig().getBoolean("witch-redstone-drop", false)) {
            Listener l = new WitchRedstoneListener(this);
        ...
        if (getConfig().getBoolean("ender-pearl-dimension", false)) {
            Listener l = new EnderPearlListener(this);
            getServer().getPluginManager().registerEvents(l, this);
            activeListeners.add(l);
        ...
        if (getConfig().getBoolean("hostile-mob-cap-70", false)) {
            Listener l = new MobCapListener(this);
            getServer().getPluginManager().registerEvents(l, this);
            activeListeners.add(l);
            getLogger().info("实验功能已启用: 敌对生物生成上限锁定");
        }

        // 注册指令（空指针保护：yml 缺失时不会炸服）
        org.bukkit.command.PluginCommand cmd = getCommand("portal121compat");
        if (cmd != null) {
            cmd.setExecutor(new CommandHandler(this));
        } else {
            getLogger().warning(
                    "无法注册指令 /portal121compat，请检查 plugin.yml 是否在插件 JAR 中");
        }
